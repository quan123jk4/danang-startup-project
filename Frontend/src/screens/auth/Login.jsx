...
const Login = () => {
  ...
  const { loginContext } = useAuth(); // Lấy hàm báo tin đăng nhập
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // BÁO CHO HỆ THỐNG BIẾT ĐÃ ĐĂNG NHẬP THÀNH CÔNG
      loginContext(response.data.user, response.data.token);

      // CHUYỂN HƯỚNG VỀ TRANG CHỦ MƯỢT MÀ (KHÔNG RELOAD)
      navigate("/");
    } catch (err) {
  ...
  // === PHẦN RETURN GIAO DIỆN CHUẨN CỦA ÔNG (Giữ nguyên 100%) ===
  return (
    <div className="flex min-h-screen w-full bg-[#fcfdfe] font-sans overflow-hidden">
      ...
      <button
        onClick={() => navigate("/")}
        className="absolute top-10 right-10 flex items-center gap-2 text-slate-400 hover:text-[#C4391D] transition-all group z-20 cursor-pointer active:scale-95"
      >
        <span className="text-[11px] font-bold uppercase tracking-widest pointer-events-none">
          Trang chủ
        </span>
        <FontAwesomeIcon
          icon={faArrowUpRightFromSquare}
          className="text-[10px] transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300"
        />
      </button>
      ...
      <div className="flex w-full flex-col justify-center px-8 md:px-24 lg:w-1/2">
        <div className="mx-auto w-full max-w-[420px]">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <button
              disabled={loading}
              className={`w-full rounded-2xl py-4 font-bold text-white shadow-xl transition-all transform active:scale-[0.99] ${loading ? "bg-slate-400 cursor-not-allowed" : "bg-[#1e293b] hover:bg-black"}`}
            >
